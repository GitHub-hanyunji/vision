...
def main(myWindow,args):
    ...
    if args.test_only:
        ...
        evaluate(model, data_loader_test, device=device, metric_logger=metric_logger)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_loss, train_acc = train_one_epoch(model, optimizer, data_loader, device, epoch, args.print_freq, metric_logger=metric_logger, scaler=scaler)
        ...
        confmat = evaluate(model, data_loader_test, device=device, metric_logger=metric_logger)
        iou_type = "bbox"
        area = "all"
        maxDets = "100"        
        v_iou_50 = confmat.coco_eval[iou_type][area][maxDets].stats[1]
        train_confmat = evaluate(model, data_loader, device=device, metric_logger=metric_logger)
        t_iou_50 = train_confmat.coco_eval[iou_type][area][maxDets].stats[1]
        
        print("v_iou_50 : ", v_iou_50)
        print("t_iou_50 : ", t_iou_50)
        
        #rec_valid[0].append(val_loss)  # 현재 에포크의 검증 손실을 리스트에 저장
        ...
        myWindow.plot(x_arr,rec_valid, rec_train)
